...
def build_checkpoint(path):
    ...
    for key, value in reference.state_dict().items():
        ...
        if key.endswith(".mlp.experts.gate_up_proj"):
            ...
            tensors[base + "switch_mlp.gate_proj.weight"] = array[:, :CFG["moe_intermediate_size"]].copy()
            tensors[base + "switch_mlp.up_proj.weight"] = array[:, CFG["moe_intermediate_size"]:].copy()
            continue
        ...
        if key.endswith("ple_embedding.ngram_embedding.weight"):
            ...
            for shard in range(CFG["split_ngram_parts"]):
                name = key.replace("ngram_embedding.weight", f"ngram_embedding.shard_{shard}.weight")
    ...
    (path / "config.json").write_text(json.dumps({
        "model_type": "qwen4_exp", "text_config": CFG, "vision_config": {},
    }))
    return reference
...
def test_full_streamed_tiny_model_matches_transformers(tmp_path):
    ...
    streamed, _ = load_model(tmp_path, expert_budget_bytes=1 << 20,
                             ple_budget_bytes=1 << 20, io_workers=4,
                             prefetch_experts=1, prefetch_ple=1)
